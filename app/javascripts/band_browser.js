...
var buildArtistDiv = function( artist ) {
  ...
  document.getElementById( 'artist-container' ).appendChild( node );
};
...
var albumTransition = function( artistDiv ) {
  var artistContainer = document.getElementById( 'artist-container' );
  var childrenArray = Array.from( artistContainer.children );

  childrenArray.forEach(
    function( child ) {
      if ( child.dataset.id !== artistDiv.dataset.id ) {
        removeArtistDiv( child );
      }
    }
  );

  getAlbums( artistDiv.dataset.id );
};
...
document.addEventListener( 'click', function( e ) {
  ...
  else if ( e.target && e.target.parentElement.classList.value.includes( 'artist' ) ) {
    albumTransition( e.target.parentElement );
  }
} );
